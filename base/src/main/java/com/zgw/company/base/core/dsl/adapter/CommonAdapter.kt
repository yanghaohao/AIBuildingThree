package com.zgw.company.base.core.dsl.adapter
...
import com.zgw.company.base.core.ext.event

class CommonAdapter<D>(create: CommonMgr<D>.() -> Unit) :
    RecyclerView.Adapter<CommonAdapter.CommonViewHolder<D>>() {
        ...
        if (acrobatItem.hasEvent()) {
            view.event({
                acrobatItem.click?.apply {
                    this(
                        commonMgr.data[viewHolder.adapterPosition],
    ...
    override fun onBindViewHolder(
        holder: CommonViewHolder<D>,
        position: Int,
        payloads: MutableList<Any>
    ) {
        if (payloads.isNotEmpty()) {
            holder.acrobatItem.showItem(
                commonMgr.data[position],
                position,
                holder.itemView,
                payloads
            )
    ...
    private inner class DiffCallback(private var mOldData: List<D>, private var mNewData: List<D>) :
        DiffUtil.Callback() {
        ...
        override fun areContentsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
            return mOldData.get(oldItemPosition).toString()
                .equals(mNewData.get(newItemPosition).toString())
        }
    }

    class CommonViewHolder<D>(view: View, val acrobatItem: CommonItem<D>) :
        ...
        fun onClick(c: (Int) -> Unit) {
            if (acrobatItem.hasEvent()) {
                throw IllegalStateException("item has inner com.zgw.company.base.core.ext.event!!!")
            }
            click = c
        }
